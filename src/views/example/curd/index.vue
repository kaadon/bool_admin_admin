<template>
    <div class='app-container'>
                    ...
                    <template v-if='searchExpand'>
                        <el-col :md='6' :sm='12'>
                            <el-form-item label='星期:' prop='week.value'>
                                <el-select v-model='queryParams.week.value' placeholder='请选择' clearable>
                                    <el-option v-for='(item, index) in week' :key='index' :label='item.label'
                                               :value='item.value' />
                                </el-select>
                            </el-form-item>
                        </el-col>
                        <el-col :md='6' :sm='12'>
                            <el-form-item label='标志:' prop='flag.value'>
                                <el-select v-model='queryParams.flag.value' placeholder='请选择' clearable>
                                    <el-option v-for='(item, index) in flag' :key='index' :label='item.label'
                                               :value='item.value' />
                                </el-select>
                            </el-form-item>
                        </el-col>
                        <el-col :md='6' :sm='12'>
                            <el-form-item label='性别:' prop='genderdata.value'>
                                <el-select v-model='queryParams.genderdata.value' placeholder='请选择' clearable>
                                    <el-option v-for='(item, index) in genderdata' :key='index' :label='item.label'
                                               :value='item.value' />
                                </el-select>
                            </el-form-item>
                        </el-col>
                        <el-col :md='6' :sm='12'>
                            <el-form-item label='爱好:' prop='actdata.value'>
                                <el-select v-model='queryParams.actdata.value' placeholder='请选择' clearable>
                                    <el-option v-for='(item, index) in ac' :key='index' :label='item.label'
                                               :value='item.value' />
                                </el-select>
                            </el-form-item>
                        </el-col>
                        ...
                        <el-col :md='6' :sm='12'>
                            <el-form-item label='开始时间戳:' prop='startdate.value'>
                                <el-date-picker v-model='queryParams.startdate.value' type='datetime'
            ...
            <el-row :gutter='10' class='mb8'>
                <el-col :span='1.5'>
                    <el-button type='primary' plain icon='el-icon-plus' size='mini' @click='open'
                               v-hasPermi='$api.example.demo.add'>
                        添加
                    </el-button>
                </el-col>
                <el-col :span='1.5'>
                    <el-button type='danger' plain icon='el-icon-delete' size='mini' :disabled='multiple'
                               @click='handleDelete' v-hasPermi='$api.example.demo.delete'>删除
                    </el-button>
                </el-col>
                <el-col :span='1.5'>
                    <el-button type='warning' plain icon='el-icon-download' size='mini' v-loading='exportLoading'
                               @click='handleExport' v-hasPermi='$api.example.demo.export'>导出
                    </el-button>
                </el-col>
                    ...
                    <el-table-column label='操作' width='180'>
                        <template slot-scope='scope'>
                            <el-button size='mini' type='text' icon='el-icon-edit' @click='handleEdit(scope.row)'
                                       v-hasPermi='$api.example.demo.edit'>编辑
                            </el-button>
                            <el-button size='mini' type='text' icon='el-icon-tickets' @click='handleDetail(scope.row)'
                                       v-hasPermi='$api.example.demo.find'>详情
                            </el-button>
                            <el-button size='mini' type='text' icon='el-icon-delete' @click='handleDelete(scope.row)'
                                       v-hasPermi='$api.example.demo.delete'>删除
                            </el-button>
                        </template>
                    </el-table-column>
                </template>
        ...
        <el-dialog :visible='editOpen' :before-close='beforeClose' :fullscreen='isFullScreen'>
            ...
            <EditForm ref='editForm' :findFormData='findFormData' :editFormParams='editFormParams' :disabled='disabled'
                      @submitForm='submitForm' @editFormCancel='beforeClose' />
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        statusChange(row) {
            return this.request.post(this.api.status, {
