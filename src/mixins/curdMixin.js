    ...
    methods: {
            ...
            this.$confirm('是否确认导出所有数据项?', "警告", {
                ...
                .then(() => {
                    ...
                    return this.request.post(this.api.export, {
                        ...this.pageInfo, ...this.formatQueryParams(this.queryParams), fields: JSON.stringify(fields)
                    })
                })
                .then(res => {
                    window.open("/admin/ajax/download?path=" + res.data.path)
                }).finally(() => {
        ...
        statusChange(row) {
            if (row.switch_target.indexOf('.') === -1 && row.switch_target === 'status') {
                this.request.post(this.api.status, {
                    id: row[this.primaryKey], [row.switch_target]: row.new_switch
                }).then(() => {
                    this.initIndex()
                })
            } else {
